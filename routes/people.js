...
router.post("/", async (req, res, next) => {
  try {
    await db.execute("INSERT INTO person (name, alive) VALUES (?, ?)", [
      req.body.name,
      true,
    ]);

    req.flash("success", "Nova pessoa registrada");
    res.redirect("/people");
  } catch {
    req.flash("error", "Erro ao registrar uma nova pessoa");
    next(err);
  }
});
...
router.delete("/:id", async (req, res, next) => {
  try {
    await db.execute("DELETE FROM person WHERE id = ?", [req.params.id]);

    req.flash("success", "Pessoa removida");
    res.redirect("/people");
  } catch {
    req.flash("error", "Erro ao remover uma pessoa");
    next(err);
  }
});
